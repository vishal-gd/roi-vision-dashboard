...
import { ArrowLeft, Building2, Server, Shield, ChevronRight, ChevronDown, Search, Package } from "lucide-react";
...
function deriveProductCategories(account: CloudAccount): { category: string; count: number }[] {
  ...
  // Return only top 5
  return all.filter(c => c.count > 0).sort((a, b) => b.count - a.count).slice(0, 5);
}
...
export function AccountGovernance({ filteredAccounts }: AccountGovernanceProps) {
  ...
  // ===== INVENTORY / CLOUD ACCOUNT DETAIL — TOP 5 CATEGORIES WITH CHARTS =====
    ...
    return (
      <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} className="glass-card rounded-xl p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{selectedAccount.tenantName} · Top 5 Product Categories</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <ReportDownload
              ...
              rows={chartData.map((c) => [c.category, c.count, c.percentage + "%"])}
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          ...
          <div className="rounded-lg bg-secondary/50 p-3 text-center">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Top Categories</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          <div>
            <h3 className="text-xs font-semibold mb-2">Top 5 Product Categories</h3>
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, bottom: 5, left: 5 }}>
                  <XAxis type="number" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} />
                  <YAxis dataKey="category" type="category" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }} width={100} />
          ...
          <div>
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={chartData} cx="50%" cy="50%" outerRadius={80} innerRadius={40} dataKey="count"
                    label={({ category, percentage }) => `${category?.slice(0, 12)} ${percentage}%`}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-xl">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {/* Mini KPI badges when collapsed */}
          {!expanded && (
